...
import com.example.krigingweb.Interpolation.Core.StatusManage;
...
class TaskGenerator implements StatusManage {
    ...
    public TaskGenerator(
        TaskStore taskStore, LandService landService
    ) {
    ...
    private boolean search(){
        return this.rectangleSearcher.search((RectangleSearcher.Rectangle rectangle) -> {
            /* 根据矩形框随机选择一个未插值地块 */
            UUID landID = this.landService.getRandomLand(rectangle);
            if(landID != null){
                final int pointsNum = 350;
                ...
                if(predictDistance != null){
                    ...
                    /* 根据最长距离的1/10生成缓冲区 */
                    double correctDistance = -maxDistance * 0.1;
